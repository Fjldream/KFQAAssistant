...
# 验证向量库和 embedding 模型只会初始化一次，后续调用复用同一个对象。
def test_create_vector_store_reuses_cached_instance(monkeypatch):
    calls = {"embeddings": 0, "vector_store": 0}
    ...
    # 记录 embedding 模型创建次数，避免测试加载真实模型。
    def fake_create_embeddings(model_name: str):
        calls["embeddings"] += 1
        return {"model_name": model_name}

    clear_factory_caches()
    monkeypatch.setattr(factory, "create_embeddings", fake_create_embeddings)
    ...
    assert calls == {"embeddings": 1, "vector_store": 1}
...
def test_clear_rag_factory_cache_resets_cached_instances(monkeypatch):
    ...
    assert calls == {"embeddings": 2, "vector_store": 2}
